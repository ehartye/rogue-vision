...
`combatEvents(run)` reports transient resolved strikes, missed attacks, pulses and actual hull removed, independently of saved state and RNG. `src/combat-effects.js` animates those records for at most 650 ms without blocking input. Effects clip to currently visible walkable cells, below future attack warnings and enemy status. Damage numbers aggregate per square; full-health enemies also show bars. Reduced motion uses static impact marks and numbers. New actions, menus, backgrounding and reload clear the effect rather than replaying it. These are presentations of existing attacks, not additional weapons or spells.
