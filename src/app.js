...
function missionSidebar(hostiles, threatened) {
  ...
  if (screen !== "mission") {
    return `<aside class="sidebar action-sidebar" aria-label="Actions"><div class="side-label">TIME HELD</div><div class="side-actions">${button("pulse", "Pulse", p.charges ? `${p.charges}/${p.maxCharges} charges<br>${p.pulseDamage} dmg · ${p.pulseRange} tiles` : "No charges", true).replace('data-action="pulse"', `data-action="pulse" aria-disabled="${p.charges === 0}"`)}${button("wait", run.relay?.progress < 3 && p.x === run.relay.x && p.y === run.relay.y ? "Tune relay" : "Wait", run.relay?.progress < 3 && p.x === run.relay.x && p.y === run.relay.y ? `${3 - run.relay.progress} turns left · no charge` : "Spend 1 turn")}${button("more", "More", "Build & sound")}</div>${threat}</aside>`;
  }
  return `<aside class="sidebar"><div class="side-label">PULSE</div><div class="pulse-dots" aria-label="${p.charges} of ${p.maxCharges} charges">${"◆".repeat(p.charges)}${"◇".repeat(p.maxCharges - p.charges)}</div><div class="small muted">Pinch for actions</div><div class="route">${DISTRICTS.map((district, i) => `<div class="route-stop ${i === run.floor ? "current" : i < run.floor ? "done" : ""}"><i></i>${district.name}</div>`).join("")}</div>${threat}<div class="side-bottom">${run.floor === 1 && run.landmark ? "⌂ Dragon Gate" + (run.relay ? "<br>▥ Lantern relay" : "") : run.floor === 3 && run.landmark ? "▱ Registration" : run.floor === 2 && run.landmark ? "▰ Cable car" : run.floor === 0 && run.landmark ? "⌂ Ferry Building" : run.relay ? "▥ Lantern relay" : "◎ Landmark"}${run.floor === 1 && run.landmark && run.relay ? "<br>◇ Signal · + Hull<br>⇩ Uplink" : "<br>◇ Signal<br>+ Hull kit<br>⇩ Uplink"}</div></aside>`;
}
