...
test("cable car identifies the existing boarding choice and retains visited art offline", async ({
  page,
  context,
}) => {
  ...
  await expect(page.locator(".map")).toHaveAttribute(
    "aria-label",
    /Powell cable car.*Enter its square/,
  );
  await expect(page.locator(".side-bottom")).toContainText("Cable car");
  ...
  await expect(
    page.getByRole("heading", { name: "Powell cable car" }),
  ).toBeVisible();
  await expect(
    page.getByText("Repair up to 5 hull · attract 2 runners"),
  ).toBeVisible();
