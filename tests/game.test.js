...
test("seeded maps are repeatable, varied, and every walkable tile is reachable", () => {
  ...
  for (let seed = 1; seed <= 120; seed++) {
    ...
    for (const [x, y] of queue)
      for (const [dx, dy] of [
        ...
        const nx = x + dx,
          ny = y + dy,
          key = `${nx},${ny}`;
        if (s.tiles[ny]?.[nx] === 0 && !seen.has(key)) {
          seen.add(key);
          queue.push([nx, ny]);
        }
      }
    ...
    const occupants = [...s.enemies, ...s.items, s.exit, s.player].map(
      (p) => `${p.x},${p.y}`,
    );
...
test("bump combat and marked enemy strikes permit dodging", () => {
  const s = arena();
  s.enemies = [
    { id: 1, kind: "husk", x: 6, y: 5, hp: 5, maxHp: 5, intent: [], stun: 0 },
  ];
  act(s, "right");
...
test("kills recharge pulses and siphon restores health", () => {
  ...
  s.kills = 2;
  s.enemies = [
    { id: 1, kind: "husk", x: 6, y: 5, hp: 1, maxHp: 4, intent: [], stun: 0 },
  ];
  act(s, "right");
...
test("save roundtrip preserves deterministic continuation and rejects damaged data", () => {
  ...
  for (const bad of [
    ...
    "null",
    '{"version":99}',
    JSON.stringify({ version: 1, state: { ...s, tiles: [] } }),
  ])
    assert.equal(decodeSave(bad), null);
  const invalid = structuredClone(s);
  invalid.player.hp = NaN;
  assert.equal(decodeSave(encodeSave(invalid)), null);
});
test("the Conductor warning leaves a one-swipe escape route", () => {
  const s = arena();
  s.enemies = [
    {
      id: 1,
  ...
  const safe = [
    ...
    ([, x, y]) => !s.enemies[0].intent.some((p) => p.x === x && p.y === y),
  );
  assert.ok(safe, "The telegraph must allow a legal one-step dodge");
  act(s, safe[0]);
  assert.equal(s.player.hp, hp);
});
